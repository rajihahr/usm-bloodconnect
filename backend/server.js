...
app.post("/sign-in", (req, res) => {
  ...
  // Query donor table
  const donorQuery =
    "SELECT donorID AS id, donorName AS name, donorEmail AS email, donorPassword AS password, 'donor' AS role FROM donor WHERE donorEmail = ? AND donorPassword = ?";
  db.query(donorQuery, [email, password], (donorErr, donorData) => {
    ...
    const medicalStaffQuery =
      "SELECT staffID AS id, staffName AS name, staffEmail AS email, staffPassword AS password, 'medical-staff' AS role FROM medicalStaff WHERE staffEmail = ? AND staffPassword = ?";
    db.query(medicalStaffQuery, [email, password], (staffErr, staffData) => {
      ...
      // Query admin table
      const adminQuery =
        "SELECT adminID AS id, adminName AS name, adminEmail AS email, adminPassword AS password, 'admin' AS role FROM admin WHERE adminEmail = ? AND adminPassword = ?";
...
app.post("/sign-up", (req, res) => {
  ...
  db.query(checkEmailSql, [donorEmail], (err, data) => {
    ...
    if (data.length > 0) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "The email address already exists." });
    }

    // If email does not exist, insert the new donor
    const sql =
      "INSERT INTO donor (donorName, donorEmail, donorPassword, donorDOB) VALUES (?, ?, ?, ?)";
    db.query(
      sql,
      [donorName, donorEmail, donorPassword, donorDOB],
      (err, data) => {
        if (err) {
          console.error("Error inserting data:", err); // Log the error
          return res.status(500).json({
            success: false,
            message: "Error adding donor to database.",
          });
        }
